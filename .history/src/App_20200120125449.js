...
class App extends Component {
  ...
  handleClick = (id, nombre) => {
    // this.setState({ loading: true })
    // const request = `{"dato":${id}}`
    // console.log(request)
    const config = {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ "dato": 9 })
    };
    fetch('http://10.255.11.201:8090/datos/insert', config)
      .then(res => { return res.json() })
      .then(data => console.log(data))
      .then(data => {
        this.setState({
          services: data.tiempoAireElectronico,
          service: data.tiempoAireElectronico[0],
          // loading: false
        })
      }).catch((err) => {
        console.log(err);
          ...
          <div className="col">
            <div className={`cards-slider active-slide-${service._id}`}>
              <div className="cards-slider-wrapper" style={{
                ...
                {
                  services.map(service => <Card key={service._id} service={service} event={() => this.handleClick(9, service.nombre)} />)
                }
              </div>
            </div>
          </div>
